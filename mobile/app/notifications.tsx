...
export default function NotificationsScreen() {
  ...
  useEffect(() => {
    api.get<Notification[]>('/api/notifications').then(({ data }) => {
  ...
  return (
    <View style={styles.container}>
      {loading ? <PageLoader text="Loading notifications..." /> : (
        <FlatList
          data={notifications}
          ...
          renderItem={({ item }) => (
            <Card style={[styles.card, !item.read && styles.unread]}>
              ...
              <View style={styles.info}>
                ...
                <Text style={styles.time}>{new Date(item.createdAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })}</Text>
              </View>
            </Card>
          )}
        />
...
const styles = StyleSheet.create({
  ...
  card: { marginBottom: Spacing.sm, flexDirection: 'row' as const, alignItems: 'flex-start' as const, gap: Spacing.md },
  unread: { backgroundColor: '#f0f4ff' },
  dotWrap: { width: 8, paddingTop: 6 },
  dot: { width: 8, height: 8, borderRadius: 4, backgroundColor: Colors.accent },
  info: { flex: 1, minWidth: 0 },
  title: { fontSize: FontSize.md, fontWeight: '500', color: Colors.ink, marginBottom: 2 },
  titleBold: { fontWeight: '700' },
  body: { fontSize: FontSize.base, color: Colors.muted, lineHeight: 18, marginBottom: Spacing.xs },
